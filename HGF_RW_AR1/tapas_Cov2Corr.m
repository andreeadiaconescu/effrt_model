...
if max(max(abs(Cov'-Cov))) >= eps % ...but give some tolerance, e.g. *machine precision
    nEpsMax = 10; % max number of machine precision before error is prompted
...
if any(isinf(Cov(:))) || any(isnan(Cov(:))) || any(eig(Cov)<=0)
    error('Input matrix is not positive-definite or cannot be assessed because isinf: %d, isnan: %d\n', any(isinf(Cov(:))), ...
        any(isnan(Cov(:)))');
end
