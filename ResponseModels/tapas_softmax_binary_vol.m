function logp = tapas_softmax_binary_vol(r, infStates, ptrans)
...
if size(r.u,2) == 1
    ...
    logp(not(ismember(1:length(logp),r.irr))) = -log(1+exp(-beta.*(2.*x-1).*(2.*y-1)));
end
...
if size(r.u,2) == 3
    % Calculate log-probabilities for non-irregular trials
    logp(not(ismember(1:length(logp),r.irr))) = -log(1+exp(-beta.*(r1.*x-r0.*(1-x)).*(2.*y-1)));
end
